...
class _GameScreenState extends State<GameScreen> {
  ...
  Future<void> _initGame() async {
    ...
    final result = generator.generate(widget.difficulty);

    _initialPuzzle = result.puzzle.toValues();
  ...
  void _onMouseEnter(PointerEvent _) {
    if (_isTimerStopped) {
      setState(() => _isTimerStopped = false);
    }
  }
          ...
          content: Column(
            ...
            children: [
              SwitchListTile(
                ...
                value: _settings.autoRemoveNotes,
                onChanged: (v) {
                  setDialogState(() => _settings.autoRemoveNotes = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                ...
                onChanged: (v) {
                  setDialogState(() => _settings.highlightSameNumbers = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                ...
                value: _settings.highlightRelatedCells,
                onChanged: (v) {
                  setDialogState(() => _settings.highlightRelatedCells = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                ...
                value: _settings.highlightCandidates,
                onChanged: (v) {
                  setDialogState(() => _settings.highlightCandidates = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                title: const Text('显示剩余数量'),
                value: _settings.showRemainingCount,
                onChanged: (v) {
                  setDialogState(() => _settings.showRemainingCount = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                ...
                onChanged: (v) {
                  setDialogState(() => _settings.showTimer = v);
                  setState(() {});
                },
              ),
              SwitchListTile(
                ...
                value: _settings.autoPauseOnLeave,
                onChanged: (v) {
                  setDialogState(() => _settings.autoPauseOnLeave = v);
                  setState(() {});
                },
              ),
            ],
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('难度：${widget.difficulty.name}'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(_drawingMode ? Icons.draw : Icons.draw_outlined),
            tooltip: '画图工具',
            onPressed: _toggleDrawingMode,
            color: _drawingMode ? AppTheme.primaryColor : null,
          ),
          IconButton(
                  ...
                  Padding(
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: DrawingToolbar(
                        ...
                        onToolChanged: (t) =>
                            setState(() => _drawingTool = t),
                        onColorChanged: (c) =>
                            setState(() => _drawingColor = c),
                        onUndo: () {
                          if (_drawingElements.isNotEmpty) {
                            setState(() {
                              _drawingElements = List.from(_drawingElements)
                                ..removeLast();
                            });
                          }
                        },
                        onClear: () =>
                            setState(() => _drawingElements = []),
                      ),
                    ),
                  ),
                Expanded(
                  child: Center(child: _buildBoardArea()),
                ),
                const SizedBox(height: 8),
                if (!_isPaused)
                  NumberPad(
                    onNumberTap: _onNumberInput,
                    onDelete: _onDelete,
                    onNotesToggle: _onNotesToggle,
                    onHint: _onHint,
                    onUndo: _onUndo,
                    onAutoNotes: _onAutoNotes,
                    isNotesMode: _gameState.isNotesMode,
                    remainingCounts: _settings.showRemainingCount
                        ? _gameState.board.getRemainingCounts()
                        : const {},
                  ),
    ...
    return Stack(
      children: [
        SudokuBoardWidget(
          ...
          highlightCandidates: _settings.highlightCandidates,
        ),
        Positioned.fill(
          child: DrawingOverlay(
            ...
            currentTool: _drawingTool,
            currentColor: _drawingColor,
            elements: _drawingElements,
            onElementsChanged: (e) =>
                setState(() => _drawingElements = e),
          ),
        ),
  ...
  Widget _buildStatusBar() {
    final timerIcon = _isPaused
        ? Icons.play_arrow_rounded
        : _isTimerStopped
            ? Icons.play_arrow_rounded
            : Icons.pause_rounded;
    final timerColor = (_isPaused || _isTimerStopped)
        ...
        children: [
          if (_settings.showTimer)
            GestureDetector(
              ...
              child: _StatusItem(
                icon: timerIcon,
                label: _formatDuration(_gameState.elapsed),
                color: timerColor,
              ),
            ),
          _StatusItem(
            icon: Icons.close,
            label: '${_gameState.mistakes}/${GameState.maxMistakes}',
            color: _gameState.mistakes > 0 ? AppTheme.errorColor : null,
          ),
          _StatusItem(
            icon: Icons.lightbulb_outline,
            label: '${_gameState.hintsUsed}',
          ),
        ],
      ),
    );
